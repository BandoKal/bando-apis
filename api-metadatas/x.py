...
spec:
  ...
  definition: |
    ...
    externalDocs:
      url: {api_link}
"""
            # Create a directory for each API inside the root directory
            directory_name = header_name
            os.makedirs(directory_name, exist_ok=True)

            api_directory = os.path.join(directory_name, api_name.replace(' ', '-'))
            os.makedirs(api_directory, exist_ok=True)

            filename = os.path.join(api_directory, 'catalog-info.yaml')
